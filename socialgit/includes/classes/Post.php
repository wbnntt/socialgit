...
class Post {
	...
	public function submitPost($body, $user_to, $imageName) {
		...
		// if($check_empty != "") {     // wb added line below to allow photo and no text
	...
	public function loadPostsFriends($data, $limit) {
		...
		if(mysqli_num_rows($data_query) > 0) {
			...
			while($row = mysqli_fetch_array($data_query)) {
				...
				//if($user_logged_obj->isFriend($added_by)){   //wb changed this to iftrue line below and it now shows all posts
				if(true){
					...
					//Once 10 posts have been loaded, break you can change the # of posts in ajax file
					if($count > $limit) {
						break;
					}
					else {
...
<!-- holds the comments and likes -->
				<div class='newsfeedPostOptions'>
					Comments($comments_check_num)&nbsp;&nbsp;&nbsp;
			<!--		<iframe src='like.php?post_id=$id' scrolling='no'></iframe> -->
				</div>

			</div> <!-- closes string div from line 309? -->
	...
	public function loadProfilePosts($data, $limit) {
		...
		if(mysqli_num_rows($data_query) > 0) {
			...
			while($row = mysqli_fetch_array($data_query)) {
				...
				?>
				<script>
				function toggle<?php echo $id; ?>() { // wb added event between the brackets - didnt help
